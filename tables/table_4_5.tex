\begin{table}[!htbp]
\caption{SOH estimation errors of different models on the Oxford dataset.}
\label{tab:4-5}
\centering
\zihao{-5}
\setlength{\tabcolsep}{2.2pt}
\renewcommand{\arraystretch}{1.02}
\begin{ThreePartTable}
\begin{tabularx}{0.98\linewidth}{>{\raggedright\arraybackslash}p{0.13\linewidth}>{\raggedright\arraybackslash}p{0.24\linewidth}>{\centering\arraybackslash}p{0.11\linewidth}>{\centering\arraybackslash}p{0.11\linewidth}>{\centering\arraybackslash}p{0.14\linewidth}>{\centering\arraybackslash}p{0.11\linewidth}}
\toprule
Cell & Method & $R^2$ & MAE & MAPE & RMSE \\
\midrule
Cell3   & MS-AgentNet       & \textbf{0.99668} & \textbf{0.00280} & \textbf{0.00333} & \textbf{0.00351} \\
        & CNN-Transformer   & 0.99523          & 0.00335          & 0.00396          & 0.00420          \\
        & CNN-LSTM          & 0.99501          & 0.00343          & 0.00404          & 0.00429          \\
        & Transformer       & 0.99472          & 0.00362          & 0.00423          & 0.00440          \\
        & LSTM              & 0.99529          & 0.00327          & 0.00390          & 0.00418          \\
\addlinespace[0.5pt]
Cell4   & MS-AgentNet       & \textbf{0.97217} & \textbf{0.00814} & \textbf{0.00960} & \textbf{0.00873} \\
        & CNN-Transformer   & 0.96942          & 0.00819          & 0.00969          & 0.00914          \\
        & CNN-LSTM          & 0.96712          & 0.00871          & 0.01026          & 0.00949          \\
        & Transformer       & 0.96931          & 0.00820          & 0.00966          & 0.00916          \\
        & LSTM              & 0.96267          & 0.00937          & 0.01103          & 0.01011          \\
\addlinespace[0.5pt]
Cell5   & MS-AgentNet       & \textbf{0.99291} & \textbf{0.00302} & \textbf{0.00343} & \textbf{0.00353} \\
        & CNN-Transformer   & 0.99002          & 0.00344          & 0.00389          & 0.00417          \\
        & CNN-LSTM          & 0.98912          & 0.00351          & 0.00397          & 0.00434          \\
        & Transformer       & 0.98727          & 0.00391          & 0.00437          & 0.00468          \\
        & LSTM              & 0.99036          & 0.00355          & 0.00400          & 0.00412          \\
\addlinespace[0.5pt]
Cell6   & MS-AgentNet       & \textbf{0.96968} & \textbf{0.00593} & \textbf{0.00705} & \textbf{0.00842} \\
        & CNN-Transformer   & 0.96442          & 0.00646          & 0.00764          & 0.00911          \\
        & CNN-LSTM          & 0.96226          & 0.00650          & 0.00771          & 0.00939          \\
        & Transformer       & 0.96402          & 0.00696          & 0.00815          & 0.00917          \\
        & LSTM              & 0.95868          & 0.00626          & 0.00747          & 0.00983          \\
\addlinespace[0.5pt]
Cell7   & MS-AgentNet       & 0.97412          & 0.00802          & 0.00923          & 0.00829          \\
        & CNN-Transformer   & \textbf{0.97781} & 0.00731          & 0.00841          & \textbf{0.00766} \\
        & CNN-LSTM          & 0.97081          & 0.00841          & 0.00964          & 0.00880          \\
        & Transformer       & 0.97750          & \textbf{0.00719} & \textbf{0.00830} & 0.00769          \\
        & LSTM              & 0.96768          & 0.00886          & 0.01016          & 0.00926          \\
\addlinespace[0.5pt]
Cell8   & MS-AgentNet       & 0.99407          & 0.00389          & 0.00444          & 0.00472          \\
        & CNN-Transformer   & 0.99434          & 0.00384          & 0.00442          & 0.00462          \\
        & CNN-LSTM          & 0.99207          & 0.00445          & 0.00508          & 0.00548          \\
        & Transformer       & \textbf{0.99528} & \textbf{0.00326} & \textbf{0.00377} & \textbf{0.00422} \\
        & LSTM              & 0.99115          & 0.00474          & 0.00539          & 0.00579          \\
\addlinespace[1pt]
Average & MS-AgentNet       & \textbf{0.98327} & \textbf{0.00530} & \textbf{0.00618} & \textbf{0.00620} \\
        & CNN-Transformer   & 0.98187          & 0.00543          & 0.00633          & 0.00649          \\
        & CNN-LSTM          & 0.97940          & 0.00584          & 0.00678          & 0.00696          \\
        & Transformer       & 0.98135          & 0.00552          & 0.00641          & 0.00655          \\
        & LSTM              & 0.97764          & 0.00601          & 0.00699          & 0.00722          \\
\bottomrule
\end{tabularx}
\begin{tablenotes}[flushleft]
\footnotesize
\item[] \parbox[t]{0.98\linewidth}{Note: Results are averaged over five independent runs. The best results are shown in bold.}
\end{tablenotes}
\end{ThreePartTable}
\end{table}
